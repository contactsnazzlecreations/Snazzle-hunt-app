// Snazzle MYSTIC/UI v228 — geverifieerde atlas met gerichte, lichte reparaties.
const VERSION='228.0-smooth';
...
function installStyle(){
  ...
  s.textContent=`
    ...
    #sc2Grid .sc2-card[data-mystic-v227='1'] .sc2-media,#sc2VaultGrid .sc2-card[data-mystic-v227='1'] .sc2-media,#sc2List .sc2-row[data-mystic-v227='1'] .sc2-thumb{position:relative!important;overflow:hidden!important;background:#171326!important}
    ...
    #sc2Grid .sc2-card.locked[data-mystic-v227='1'] .sn-mystic-v227-art,#sc2VaultGrid .sc2-card.locked[data-mystic-v227='1'] .sn-mystic-v227-art{filter:brightness(.46) saturate(.68) blur(3.2px)!important;transform:scale(1.065)!important}
    #sc2Grid .sc2-card.unlocked[data-mystic-v227='1'] .sn-mystic-v227-art,#sc2VaultGrid .sc2-card.unlocked[data-mystic-v227='1'] .sn-mystic-v227-art{filter:none!important;transform:none!important}
    #sc2Grid .sc2-card[data-mystic-v227='1'] .sc2-lock,#sc2VaultGrid .sc2-card[data-mystic-v227='1'] .sc2-lock{z-index:90!important;color:#ffe15a!important;text-shadow:0 3px 12px #000!important}
    #sc2Grid .sc2-card[data-mystic-v227='1'] .sc2-rarity,#sc2Grid .sc2-card[data-mystic-v227='1'] .sc2-num,#sc2VaultGrid .sc2-card[data-mystic-v227='1'] .sc2-rarity,#sc2VaultGrid .sc2-card[data-mystic-v227='1'] .sc2-num{z-index:91!important}
...
function fixTopMenu(){
  ...
  const year=buttons.find(b=>/jaarstand|jaar|klassement/i.test(b.textContent||''));
  const vault=buttons.find(b=>b.dataset.collectionTab==='vault'||/vault/i.test(b.textContent||''));
  if(year)year.textContent='🏆 Jaarstand';
  if(vault)vault.textContent='🔐 Vault';
  if(year&&vault){
    buttons.forEach(b=>{if(b===year||b===vault)b.removeAttribute('data-sn-v227-hidden');else b.dataset.snV227Hidden='1';});
  }
}
...
function repair(){
  installStyle();fixTopMenu();if(!cardUiExists())return 0;let count=0;
...
function queue(force=false){
  ...
  queued=true;requestAnimationFrame(()=>{queued=false;try{repair()}catch(e){console.error('MYSTIC/UI v228 repair',e)}});
}
...
document.addEventListener('snazzle:mystic-ready',()=>{queue(true);setTimeout(()=>queue(true),300);});

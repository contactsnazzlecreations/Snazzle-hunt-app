// Snazzle Special Findings v294 — aparte, betrouwbare verzameling voor algemene AR Special Snazzles.
...
const LOCAL_PLACES=[
  ...
  ['Herten',51.1806,5.9629],
  ['Maasniel',51.2007,6.0064],
  ['Asenray',51.1966,6.0459],
  ['Swalmen',51.2308,6.0369],
  ['Boukoul',51.2304,6.0715],
  ['Montfort',51.1255,5.9485],
  ['Posterholt',51.1233,6.0319],
  ['Sint Odiliënberg',51.1435,6.0003],
  ['Melick',51.1587,6.0163],
  ['Herkenbosch',51.1536,6.0663],
  ['Vlodrop',51.1337,6.0762],
  ['Linne',51.1551,5.9385],
  ['Maasbracht',51.1393,5.8866],
  ['Echt',51.1048,5.8783],
  ['Heel',51.1796,5.8948],
  ['Horn',51.2102,5.9440],
  ['Haelen',51.2350,5.9561],
  ['Heythuysen',51.2504,5.8993],
  ['Baexem',51.2267,5.8811],
  ['Neer',51.2608,5.9891]
];
const toRad=d=>d*Math.PI/180;
function kmBetween(lat1,lon1,lat2,lon2){
  const R=6371,dLat=toRad(lat2-lat1),dLon=toRad(lon2-lon1),a=Math.sin(dLat/2)**2+Math.cos(toRad(lat1))*Math.cos(toRad(lat2))*Math.sin(dLon/2)**2;
  return 2*R*Math.atan2(Math.sqrt(a),Math.sqrt(1-a));
}
function localPlaceFromCoords(item){
  if(!Number.isFinite(item.lat)||!Number.isFinite(item.lon))return'';
  const nearest=LOCAL_PLACES.map(([name,lat,lon])=>({name,km:kmBetween(item.lat,item.lon,lat,lon)})).sort((a,b)=>a.km-b.km)[0];
  return nearest&&nearest.km<=6.5?nearest.name:'';
}
...
async function reversePlace(item){
  ...
  const local=localPlaceFromCoords(item);
  if(local){
    placeCache[item.id]=local;
    savePlaceCache();
    return local;
  }
...
console.info('Snazzle Special Findings v294 lokale plaatsnamen actief');

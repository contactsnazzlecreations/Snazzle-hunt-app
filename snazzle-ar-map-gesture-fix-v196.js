...
function applyMapGesture197(){
  ...
  if(visible&&!wasVisible197){remotePoint197=null;const input=$197('#sn197Address');if(input)input.value='';const ss=$197('#sn197SearchStatus');if(ss)ss.textContent='Gebruik een volledig adres voor de nauwkeurigste plek.';}
  wasVisible197=visible;
  ...
  if(visible)lockPageZoom197();else unlockPageZoom197();
}
...
function parseCurrentPoint197(){
  ...
  if(m){
    ...
    if(Number.isFinite(lat)&&Number.isFinite(lon)) return {lat,lon,label:'huidige kaartlocatie'};
  }
  const src=$197('#sn195MapFrame')?.src||'';
  try{
    const u=new URL(src);
    const marker=decodeURIComponent(u.searchParams.get('marker')||'');
    const [lat,lon]=marker.split(',').map(Number);
    if(Number.isFinite(lat)&&Number.isFinite(lon)) return {lat,lon,label:'kaartlocatie'};
  }catch{}
  return null;
}
...
function setRemotePoint197(point){
  ...
  const frame=$197('#sn195MapFrame');
  if(frame)frame.src=mapUrl197(point.lat,point.lon);
  const st=$197('#sn195LocationStatus');
  if(st){st.className='sn195-status ok';st.textContent=`📍 ${point.lat.toFixed(6)}, ${point.lon.toFixed(6)} · gekozen adres`;
    st.dataset.sn197Lat=String(point.lat);st.dataset.sn197Lon=String(point.lon);
  }
  const ss=$197('#sn197SearchStatus');
  if(ss)ss.textContent=`✅ ${point.label||'Adres gevonden'}`;
}

async function searchAddress197(){
  const input=$197('#sn197Address');
  const status=$197('#sn197SearchStatus');
  ...
  try{
    ...
    if(!res.ok)throw new Error('Adreszoeker reageert niet.');
    const rows=await res.json();
    const hit=rows?.[0];
    if(!hit)throw new Error('Geen adres gevonden. Voeg plaatsnaam of huisnummer toe.');
    const lat=Number(hit.lat),lon=Number(hit.lon);
    if(!Number.isFinite(lat)||!Number.isFinite(lon))throw new Error('Adres gaf geen geldige locatie.');
    setRemotePoint197({lat,lon,label:hit.display_name||q});
  }catch(err){
    if(status)status.textContent='⚠️ '+(err?.message||'Adres zoeken mislukt.');
  }
}

function formData197(){
  return {
    name:($197('#snArAdminName85')?.value||'Scout Snazzle').trim(),
    number:($197('#snArAdminNumber85')?.value||'001').trim(),
    rarity:$197('#snArAdminRarity85')?.value||'RARE',
    village:$197('#snArAdminVillage85')?.value||'Montfort',
    radius:Number($197('#snArAdminRadius85')?.value||7),
    file:$197('#snArAdminImage85')?.files?.[0]||null
  };
}

async function saveMapOnly197(){
  const btn=$197('#sn197MapOnly');
  const searchStatus=$197('#sn197SearchStatus');
  const f=formData197();
  const p=parseCurrentPoint197();
  ...
  if(searchStatus)searchStatus.textContent='☁️ Snazzle op kaart opslaan…';
  try{
    ...
    const db=getFirestore(),storage=getStorage(),worldDoc=doc(db,'hunts',WORLD197);
    ...
    const point={
      ...
      lat:p.lat,lon:p.lon,accuracy:0,imageUrl,active:true,
      placement:{version:3,mode:'map-only',x:.5,y:.56,size:.34,rotation:0,placedAt:now},
    ...
    if(searchStatus)searchStatus.textContent=`✅ ${f.name} staat nu op deze kaartlocatie.`;
    toast197('✅ Snazzle via kaart geplaatst — camera niet nodig.');
    window.SnazzleArAdminV85?.refresh?.();
  }catch(err){
    console.error('Snazzle kaart-only v197',err);
    if(searchStatus)searchStatus.textContent='⚠️ '+(err?.message||'Opslaan mislukt.');
  }finally{if(btn)btn.disabled=false;}
}

function stopCamera197(){
  if(stream197){try{stream197.getTracks().forEach(t=>t.stop());}catch{}stream197=null;}
  const video=$197('#sn195Video');
  const old=video?.srcObject;
  if(old && old!==stream197){try{old.getTracks?.().forEach(t=>t.stop());}catch{}}
  if(video)video.srcObject=null;
}

function setPreview197(){
  if(previewUrl197){try{URL.revokeObjectURL(previewUrl197);}catch{}previewUrl197='';}
  const file=formData197().file,img=$197('#sn195Image'),duck=$197('#sn195Duck');
  if(!img||!duck)return;
  if(file){previewUrl197=URL.createObjectURL(file);img.src=previewUrl197;img.hidden=false;duck.hidden=true;}
  else{img.removeAttribute('src');img.hidden=true;duck.hidden=false;}
}

function showCameraStep197(){
  const map=$197('#sn195MapSection'),cam=$197('#sn195CamSection'),done=$197('#sn195DoneSection');
  if(map)map.hidden=true;if(cam)cam.hidden=false;if(done)done.hidden=true;
...
function timedCamera197(constraints,ms=9000){
  ...
  const request=navigator.mediaDevices.getUserMedia(constraints).then(s=>{
    if(timedOut){try{s.getTracks().forEach(t=>t.stop());}catch{}throw new Error('camera-timeout');}
    return s;
  });
  const timeout=new Promise((_,reject)=>setTimeout(()=>{timedOut=true;reject(new Error('camera-timeout'));},ms));
...
async function chooseRearCamera197(current){
  try{
    ...
    const rear=devices.find(d=>/back|rear|environment|achter/i.test(d.label||''));
    if(!rear)return current;
    const settings=current.getVideoTracks?.()[0]?.getSettings?.()||{};
    if(settings.deviceId===rear.deviceId)return current;
    const next=await timedCamera197({video:{deviceId:{exact:rear.deviceId}},audio:false},7000);
...
async function startCamera197(){
  const status=$197('#sn195CameraStatus'),retry=$197('#sn197RetryCamera'),video=$197('#sn195Video');
  if(retry)retry.classList.remove('show');
  stopCamera197();setPreview197();
  if(status){status.className='sn195-status';status.textContent='📷 Camera openen…';}
  try{
    if(!window.isSecureContext)throw new Error('Camera vereist een beveiligde HTTPS-verbinding.');
    if(!navigator.mediaDevices?.getUserMedia)throw new Error('Camera wordt op dit toestel niet ondersteund.');
    let s=await timedCamera197({video:true,audio:false},9000);
    s=await chooseRearCamera197(s);
    stream197=s;
    if(!video)throw new Error('Camerabeeld ontbreekt.');
    video.srcObject=s;
    await Promise.race([video.play().catch(()=>{}),sleep197(2500)]);
    if(status){status.className='sn195-status ok';status.textContent='✅ Camera actief. Sleep de Snazzle naar de juiste plek.';}
  }catch(err){
    console.error('Camera v197',err);
    stopCamera197();
    let msg='Camera kon niet openen.';
    if(err?.name==='NotAllowedError'||/permission|toegang|denied/i.test(err?.message||''))msg='Cameratoegang is geweigerd. Geef de Snazzle-app cameratoegang en probeer opnieuw.';
    else if(err?.name==='NotFoundError')msg='Geen camera gevonden op dit toestel.';
    else if(/timeout/.test(err?.message||''))msg='Camera reageerde niet binnen 9 seconden. Tik op “Camera opnieuw openen”.';
    else if(err?.message)msg=err.message;
    if(status){status.className='sn195-status err';status.textContent='⚠️ '+msg;}
    if(retry)retry.classList.add('show');
  }
}

function addUi197(){
  if(installing197)return;
  installing197=true;
  try{
    installStyle197();
    ...
    if(card && !$197('#sn197RemoteBox')){
      ...
      box.id='sn197RemoteBox';box.className='sn197-remote';
      ...
      const actions=card.querySelector('.sn195-actions');
      if(actions)card.insertBefore(box,actions);else card.appendChild(box);
      $197('#sn197Search')?.addEventListener('click',searchAddress197);
      $197('#sn197Address')?.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();searchAddress197();}});
      $197('#sn197MapOnly')?.addEventListener('click',saveMapOnly197);
    }
    const camCard=$197(`#${MODAL197} #sn195CamSection .sn195-card`);
    if(camCard && !$197('#sn197RetryCamera')){
      const retry=document.createElement('button');retry.type='button';retry.id='sn197RetryCamera';retry.className='sn197-retry';retry.textContent='📷 Camera opnieuw openen';
      const status=$197('#sn195CameraStatus');
      if(status)status.insertAdjacentElement('afterend',retry);else camCard.appendChild(retry);
...
document.addEventListener('pointerdown',event=>{
  if(!cameraButton197(event.target))return;
...
document.addEventListener('click',event=>{
  if(event.target?.closest?.('#sn195Gps,[data-nudge]'))remotePoint197=null;
  if(cameraButton197(event.target)){
    event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();
    return;
  }
  if(event.target?.closest?.('#sn195BackMap,#sn195Close,#sn195DoneClose'))stopCamera197();
},true);

...
function installStyle(){
  if($('#snArAdminV851Style')) return;
  const s=document.createElement('style');
  s.id='snArAdminV851Style';
  s.textContent=`
    #snArAdminV85{display:none}#snArAdminV85.on{display:block!important}
    .sn-ar-admin-grid{display:grid;gap:12px}.sn-ar-admin-note{padding:12px 13px;border-radius:14px;background:#fff8e5;border:2px solid #c29b5d;font-size:12px;font-weight:750;line-height:1.45}
    .sn-ar-gps-status{padding:11px 12px;border-radius:13px;background:#f2e5bc;border:2px solid #c19a5c;font-size:12px;font-weight:900}.sn-ar-gps-status.ok{background:#e4f3c6;border-color:#8cae4b;color:#315421}
    .sn-ar-gps{background:linear-gradient(#6fbe3f,#438e2c)!important;color:#fff!important}.sn-ar-image-preview{height:120px;border-radius:16px;border:2px dashed #b48d55;background:#fff8e7;display:grid;place-items:center;overflow:hidden;margin-top:8px;padding:8px;text-align:center}
    .sn-ar-image-preview img{max-width:100%;max-height:100%;object-fit:contain}.sn-ar-empty{padding:14px;text-align:center;background:#fff8e6;border:2px dashed #c4a36c;border-radius:15px;font-weight:850;color:#6a5338}
    ...
    @media(max-width:390px){.sn-ar-admin-card .row{grid-template-columns:1fr}}
  `;
...
function applyVisibility(){
  const tab=$('#snArAdminTab85'), sec=$('#snArAdminV85');
...
function install(){
  ...
  section.className='admin-section'; section.id='snArAdminV85';
  section.innerHTML=`
    <h3>AR Snazzles 📍</h3>
    <div class="sn-ar-admin-note">Loop naar de plek waar de Snazzle moet blijven staan en druk op <b>“Plaats Snazzle hier”</b>. Alleen dat vaste spelpunt wordt bewaard. De route van kinderen wordt niet opgeslagen.</div>
    <div class="sn-ar-admin-grid">
      <div class="field"><label>Naam Snazzle</label><input id="snArAdminName85" maxlength="50" value="Scout Snazzle"></div>
      <div class="row2"><div class="field"><label>Kaartnummer</label><input id="snArAdminNumber85" maxlength="12" value="001"></div><div class="field"><label>Zeldzaamheid</label><select id="snArAdminRarity85"><option>COMMON</option><option selected>RARE</option><option>GOLD</option><option>PLATINUM</option><option>BLACK</option><option>LEGENDARY</option></select></div></div>
      <div class="row2"><div class="field"><label>Dorp</label><select id="snArAdminVillage85"></select></div><div class="field"><label>Vangzone</label><select id="snArAdminRadius85"><option value="5">5 meter</option><option value="7" selected>7 meter</option><option value="10">10 meter</option><option value="15">15 meter</option></select></div></div>
      <div class="field"><label>Transparante Snazzle-afbeelding (PNG aanbevolen)</label><input id="snArAdminImage85" type="file" accept="image/png,image/webp,image/*"><div class="sn-ar-image-preview" id="snArAdminPreview85">🦆 Optioneel — zonder afbeelding gebruiken we de test-Snazzle.</div></div>
      <div class="sn-ar-gps-status" id="snArAdminStatus85">⏳ AR-opslag controleren…</div>
      <button class="save sn-ar-gps" id="snArAdminPlace85" type="button">📍 Plaats Snazzle hier</button>
    </div>
    <h3 style="margin-top:22px">Geplaatste Snazzles</h3>
    <div class="list" id="snArAdminList85"><div class="sn-ar-empty">AR-punten laden…</div></div>`;
...
function previewImage(e){
  const file=e.target.files?.[0],box=$('#snArAdminPreview85'); if(!box)return;
  if(!file){box.textContent='🦆 Optioneel — zonder afbeelding gebruiken we de test-Snazzle.';return;}
  const url=URL.createObjectURL(file); box.innerHTML=`<img src="${url}" alt="Voorbeeld Snazzle">`;
}
...
async function writeWorld(next){
  await setDoc(WORLD_DOC,{_snazzleInternalType:'arWorld',title:'[SYSTEEM] AR-WERELD',village:'snazzle-internal',description:'Interne opslag voor permanente Snazzle AR-punten',rule:'',hint:'',foundMessage:'',imageUrl:'',start:'',end:'',mode:'draft',version:3,points:next,updatedAt:new Date().toISOString(),updatedBy:adminUid},{merge:true});
  points=next; renderList(); hideSystemHunt();
}

async function placeHere(){
  const btn=$('#snArAdminPlace85'),status=$('#snArAdminStatus85');
  if(!status)return;
  if(!superAdmin){status.textContent='⚠️ Alleen de hoofdbeheerder kan AR-punten plaatsen.';return;}
  const name=($('#snArAdminName85')?.value||'').trim();
  const number=($('#snArAdminNumber85')?.value||'').trim();
  if(name.length<2){status.textContent='⚠️ Vul een naam in.';return;}
  ...
  try{
    ...
    const input=$('#snArAdminImage85'); if(input)input.value='';
    const preview=$('#snArAdminPreview85'); if(preview)preview.textContent='🦆 Optioneel — zonder afbeelding gebruiken we de test-Snazzle.';
  }catch(err){status.classList.remove('ok');status.textContent='⚠️ '+friendlyError(err);}finally{if(btn)btn.disabled=false;}
}
...
async function refreshWorld(){
  ...
  try{
    ...
    if(list)list.innerHTML=`<div class="sn-ar-empty">⚠️ ${esc(msg)}</div>`;
    if(status){status.classList.remove('ok');status.textContent='⚠️ '+msg;}
  }
}
function renderList(){
...
async function togglePoint(id){
  if(!superAdmin)return;
  try{const current=await readWorld();await writeWorld(current.map(p=>p.id===id?{...p,active:!p.active,updatedAt:new Date().toISOString()}:p));}
  catch(err){const s=$('#snArAdminStatus85');if(s)s.textContent='⚠️ '+friendlyError(err);}
}
async function deletePoint(id){
  if(!superAdmin||!confirm('Deze AR Snazzle definitief van deze plek verwijderen?'))return;
  try{const current=await readWorld();await writeWorld(current.filter(p=>p.id!==id));}
  catch(err){const s=$('#snArAdminStatus85');if(s)s.textContent='⚠️ '+friendlyError(err);}
}

onAuthStateChanged(auth,async user=>{
  adminUid=user?.uid||''; superAdmin=false;
  if(user){try{const snap=await getDoc(doc(db,'adminUsers',user.uid)),p=snap.exists()?snap.data():null;superAdmin=!!(p?.active===true&&p?.role==='superadmin');}catch{}}

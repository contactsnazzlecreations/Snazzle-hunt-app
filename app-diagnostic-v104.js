// Snazzle v104 startup diagnostic — mirrors the current app module list.
const runtimeVersion='20260826-diagnostic-v104';
const fresh=(path)=>`${path}${path.includes('?')?'&':'?'}fresh=${encodeURIComponent(runtimeVersion)}`;
window.__snazzleRuntimeVersion=runtimeVersion;
window.__snazzleFresh=fresh;
window.__snazzleDiagnosticV104=true;

function ensureSplash(){
  let splash=document.getElementById('snDiagBoot104');
  if(splash) return splash;
  splash=document.createElement('div');
  splash.id='snDiagBoot104';
  splash.innerHTML=`<div class="sn-diag-card"><div class="sn-diag-mark">🦆</div><h1>Snazzle</h1><p>Samen naar buiten</p><small id="snDiagStage104">Diagnose wordt gestart…</small><div class="sn-diag-line"><i></i></div><em>Deze controle verandert niets aan je Snazzle-gegevens.</em></div>`;
  const style=document.createElement('style');
  style.id='snDiagStyle104';
  style.textContent=`#snDiagBoot104{position:fixed;inset:0;z-index:2147483647;display:grid;place-items:center;background:radial-gradient(circle at 50% 28%,rgba(174,236,90,.22),transparent 30%),linear-gradient(180deg,#0d6844,#043a2b);color:#fff8df;font-family:system-ui,-apple-system,"Segoe UI",sans-serif;text-align:center;padding:22px}#snDiagBoot104 .sn-diag-card{width:min(88vw,390px)}#snDiagBoot104 .sn-diag-mark{width:104px;height:104px;margin:0 auto 17px;border-radius:50%;display:grid;place-items:center;background:linear-gradient(145deg,#ffe675,#f5b536);border:5px solid #805527;box-shadow:0 8px 0 #4e311b;font-size:48px}#snDiagBoot104 h1{margin:0;color:#ffd75a;font-size:42px;line-height:1}#snDiagBoot104 p{margin:13px 0 18px;font-size:20px;font-weight:900;color:#e8f4cf}#snDiagStage104{display:block;min-height:42px;padding:10px 12px;border-radius:13px;background:rgba(0,0,0,.18);border:1px solid rgba(255,231,145,.28);color:#fff0b8;font-size:13px;font-weight:850;line-height:1.35;overflow-wrap:anywhere}#snDiagBoot104 .sn-diag-line{height:6px;margin:17px auto 0;width:230px;max-width:72vw;border-radius:99px;background:rgba(255,255,255,.18);overflow:hidden}#snDiagBoot104 .sn-diag-line i{display:block;width:38%;height:100%;border-radius:99px;background:#ffe071;animation:snDiag104 1.1s ease-in-out infinite alternate}#snDiagBoot104 em{display:block;margin-top:13px;color:#dce8c9;font-size:10px;font-style:normal;font-weight:700;opacity:.82}@keyframes snDiag104{from{transform:translateX(-15%)}to{transform:translateX(180%)}}`;
...
function stage(label){
  try{localStorage.setItem('snazzleDiagLastStageV104',String(label));}catch{}
  const el=document.getElementById('snDiagStage104');
...
function finish(){stage('Controle klaar — volledige app is opgebouwd');try{localStorage.setItem('snazzleDiagLastStageV104','KLAAR');}catch{}}

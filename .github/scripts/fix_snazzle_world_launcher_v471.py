...
stories_block = '''const FALLBACK_STORIES=[
...
missions_block = '''const FALLBACK_MISSIONS=[
...
# Vervang beide standaardblokken via callable replacements zodat nieuwe regels in
# template literals niet door re.sub als escape-sequenties worden geïnterpreteerd.
js_new, n_stories = re.subn(
    r"const FALLBACK_STORIES=\[.*?\];\nconst FALLBACK_MISSIONS=",
    lambda m: stories_block + "\nconst FALLBACK_MISSIONS=",
    js,
    count=1,
    flags=re.S,
)
...
js_new, n_missions = re.subn(
    r"const FALLBACK_MISSIONS=\[.*?\];\nconst FALLBACK_TV=",
    lambda m: missions_block + "\nconst FALLBACK_TV=",
    js_new,
    count=1,
    flags=re.S,
)
...
js_new = re.sub(r"snazzle-world-hub-v47\.css\?v=\d+", 'snazzle-world-hub-v47.css?v=474', js_new, count=1)
js_path.write_text(js_new, encoding='utf-8')

app = app_path.read_text(encoding='utf-8')
app_new, n_app = re.subn(r"snazzle-world-hub-v47\.js\?v=\d+", 'snazzle-world-hub-v47.js?v=474', app, count=1)
if n_app != 1:
...
print('Snazzle Wereld v47.4: langere verhaalhoofdstukken en 3 extra opdrachten correct geplaatst')

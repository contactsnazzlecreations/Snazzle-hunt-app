// Snazzle Hunt v269 — robuuste beheerlogin zonder Firebase auth-race of eindeloos wachten.

const runtimeVersion='20260918-v269-admin-auth';
...
function installMobilePerformanceMode(){
  ...
  const style=document.createElement('style');style.id='snFastMobileV177';style.textContent=`
@media (max-width:700px),(pointer:coarse){body{animation:none!important;background-size:100% 100%!important}.logo,.logo:after,.hero:before,.hero:after,.main-action:after,.compass,.go,.photo>.live,.found.ready,.quick-menu-btn:after{animation:none!important}.quick-menu-overlay,.sheet,.event-poster-overlay{backdrop-filter:none!important;-webkit-backdrop-filter:none!important}.quick-menu-panel,.panel{will-change:transform}#snArStart,#snArZoneOpen,#snArZoneNativeOpen,#snArIntroCloseV245{touch-action:manipulation!important;pointer-events:auto!important}}
`;document.head.appendChild(style);
}
installMobilePerformanceMode();

function refreshLocalStyles(){
  document.querySelectorAll('link[rel="stylesheet"][href]').forEach(link=>{try{const url=new URL(link.getAttribute('href'),location.href);if(url.origin!==location.origin||!url.pathname.includes('/Snazzle-hunt-app/')||url.searchParams.get('fresh')===runtimeVersion)return;url.searchParams.set('fresh',runtimeVersion);link.href=url.href;}catch{}});
}
for(const href of ['./snazzle-magic-theme.css','./snazzle-enchanted-layer.css','./snazzle-professional-v53.css','./snazzle-final-polish-v59.css','./snazzle-magic-popup-v179.css']){const l=document.createElement('link');l.rel='stylesheet';l.href=fresh(href);document.head.appendChild(l);}
refreshLocalStyles();

function suppressLateStartupOverlays(){
  try{sessionStorage.setItem('snazzleProSplashSeen','1');}catch{}
  document.body?.classList.remove('sn-v59-booting');document.body?.classList.add('sn-v59-ready');document.querySelectorAll('#snV59Boot,.sn-pro-splash').forEach(el=>el.remove());
}
suppressLateStartupOverlays();window.__snazzleReleaseBoot=window.__snazzleReleaseBoot||(()=>{});

await Promise.all([safeImport('./snazzle-runtime-stability-v71.js'),safeImport('./snazzle-image-stability-v72.js'),safeImport('./snazzle-leaflet-isolation-v190.js')]);
...
await safeImport('./snazzle-clean-home-v31.js?v=269');
...
(function installHeroQuack(){
  ...
  const playQuack=()=>{const AudioCtx=window.AudioContext||window.webkitAudioContext;if(!AudioCtx)return;audioContext||=new AudioCtx();if(audioContext.state==='suspended')audioContext.resume().catch(()=>{});const now=audioContext.currentTime,master=audioContext.createGain(),filter=audioContext.createBiquadFilter(),osc1=audioContext.createOscillator(),osc2=audioContext.createOscillator();filter.type='bandpass';filter.frequency.setValueAtTime(720,now);filter.Q.setValueAtTime(.75,now);osc1.type='sawtooth';osc2.type='square';osc1.frequency.setValueAtTime(500,now);osc1.frequency.exponentialRampToValueAtTime(255,now+.16);osc1.frequency.setValueAtTime(360,now+.17);osc1.frequency.exponentialRampToValueAtTime(215,now+.31);osc2.frequency.setValueAtTime(250,now);osc2.frequency.exponentialRampToValueAtTime(135,now+.30);master.gain.setValueAtTime(.0001,now);master.gain.exponentialRampToValueAtTime(.085,now+.018);master.gain.exponentialRampToValueAtTime(.045,now+.13);master.gain.setValueAtTime(.075,now+.17);master.gain.exponentialRampToValueAtTime(.0001,now+.34);osc1.connect(filter);osc2.connect(filter);filter.connect(master);master.connect(audioContext.destination);osc1.start(now);osc2.start(now);osc1.stop(now+.35);osc2.stop(now+.35);};
...
const fastBundles=[
  ['./snazzle-ar-legacy-cleanup-v187.js','./snazzle-news-v47.js','./snazzle-news-bg-v48.js','./snazzle-news-card-bg-v49.js','./snazzle-parent-hub-v65.js'],
...
const backgroundBundles=[
  ...
  ['./image-fit.js','./snazzle-home-magic.js','./snazzle-home-magic-fix.js','./snazzle-central-visuals-v54.js?v=269','./snazzle-public-visual-publish-v64.js','./snazzle-image-recovery-v60.js','./snazzle-professional-v53.js','./snazzle-final-polish-v59.js','./snazzle-star-rewards-v67.js','./snazzle-quiet-psst-v68.js','./snazzle-input-visibility-v69.js','./snazzle-top-stability-v70.js'],
  ['./snazzle-admin-reset-v49.js','./snazzle-admin-backup-v50.js','./snazzle-admin-close-v61.js','./snazzle-admin-access-v55.js','./snazzle-admin-access-v56.js']
];

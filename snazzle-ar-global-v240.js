// Snazzle AR Global v240 — wereldwijde AR met instelbare maximale zoekstraal.
...
let maxSearchRadiusKm=DEFAULT_MAX_RADIUS_KM;
...
function radiusLabel(v=maxSearchRadiusKm){return Number(v)%1===0?`${Number(v)} km`:`${Number(v).toFixed(1)} km`;}
function clampRadius(v){const n=Number(v);return Number.isFinite(n)?Math.max(.1,Math.min(1000,n)):DEFAULT_MAX_RADIUS_KM;}

async function loadMaxRadius(){
  try{
    const snap=await getDoc(WORLD_DOC),data=snap.exists()?snap.data():{};
    maxSearchRadiusKm=clampRadius(data.arMaxSearchRadiusKm??DEFAULT_MAX_RADIUS_KM);
  }catch{maxSearchRadiusKm=DEFAULT_MAX_RADIUS_KM;}
  const input=$('#snArMaxSearchRadius240');if(input)input.value=String(maxSearchRadiusKm);
  const current=$('#snArMaxSearchCurrent240');if(current)setText(current,`Huidige maximale zoekstraal: ${radiusLabel()}`);
  return maxSearchRadiusKm;
}
async function saveMaxRadius(){
  const role=($('#adminRole')?.textContent||'').trim();
  const status=$('#snArAdminStatus85');
  if(role!=='Hoofdbeheerder'){if(status)setText(status,'⚠️ Alleen de hoofdbeheerder kan de AR-zoekstraal aanpassen.');return;}
  const input=$('#snArMaxSearchRadius240'),value=clampRadius(input?.value);
  if(input)input.value=String(value);
  try{
    await setDoc(WORLD_DOC,{arMaxSearchRadiusKm:value,updatedAt:new Date().toISOString()},{merge:true});
    maxSearchRadiusKm=value;
    const current=$('#snArMaxSearchCurrent240');if(current)setText(current,`Huidige maximale zoekstraal: ${radiusLabel()}`);
    if(status)setText(status,`✅ Maximale AR-zoekstraal opgeslagen: ${radiusLabel()}`);
  }catch(err){if(status)setText(status,'⚠️ Zoekstraal opslaan is mislukt. Probeer opnieuw.');console.error(err);}
}
...
function installMaxRadiusAdmin(){
  const grid=$('#snArAdminV85 .sn-ar-admin-grid');if(!grid)return false;
  if($('#snArMaxSearchRadiusWrap240'))return true;
  const wrap=document.createElement('div');wrap.className='field';wrap.id='snArMaxSearchRadiusWrap240';
  wrap.innerHTML=`<label>Maximale zoekstraal dichtstbijzijnde Snazzle</label><div style="display:grid;grid-template-columns:1fr auto;gap:8px;align-items:center"><input id="snArMaxSearchRadius240" type="number" min="0.1" max="1000" step="0.5" inputmode="decimal" value="${DEFAULT_MAX_RADIUS_KM}" aria-label="Maximale AR zoekstraal in kilometer"><button type="button" class="secondary" id="snArSaveMaxSearchRadius240" style="width:auto;margin:0;padding:12px 14px">Opslaan</button></div><small id="snArMaxSearchCurrent240" style="display:block;margin-top:6px;font-weight:800;line-height:1.35;color:#6b5438">Huidige maximale zoekstraal: ${DEFAULT_MAX_RADIUS_KM} km</small><small style="display:block;margin-top:3px;font-weight:750;line-height:1.35;color:#6b5438">Zoekers krijgen alleen een AR-Snazzle aangeboden als die binnen deze afstand van hun GPS-locatie ligt. De wereldkaart blijft wel alle zones tonen.</small>`;
  const gpsStatus=$('#snArAdminStatus85');
  if(gpsStatus)grid.insertBefore(wrap,gpsStatus);else grid.appendChild(wrap);
  $('#snArSaveMaxSearchRadius240')?.addEventListener('click',saveMaxRadius);
  loadMaxRadius();
  return true;
}
...
function watchAdminRole(){
  ...
  roleObserver=new MutationObserver(()=>{lockArAdminToHeadAdmin();if(($('#adminRole')?.textContent||'').trim()==='Hoofdbeheerder')loadMaxRadius();});
...
function ensureGlobalMapLink(){
  ...
  if(existing){
    if(existing.getAttribute('href')!=='./snazzle-zones.html?v=240')existing.setAttribute('href','./snazzle-zones.html?v=240');
  ...
  const link=document.createElement('a');link.id='snArZoneNativeOpen';link.className=old.className||'sn-ar-zone-btn';link.href='./snazzle-zones.html?v=240';link.textContent='🗺️ Bekijk alle Snazzle-zones';link.setAttribute('role','button');link.setAttribute('aria-label','Bekijk alle Snazzle-zones in Nederland en daarbuiten');old.replaceWith(link);return true;
}
function customizeGlobalCopy(){
  setText($('#snArLaunch .sn-ar-copy small'),'Zoek geplaatste Snazzles overal met camera en GPS');
  setText($('#snArIntro p:not(.sn-ar-privacy)'),`Bekijk op de kaart waar Snazzle-zones zijn in Nederland en daarbuiten. Start daarna AR: de app kiest automatisch de dichtstbijzijnde actieve Snazzle binnen maximaal ${radiusLabel()}.`);
...
async function refreshIntroStatus(){
  ...
  try{
    const [points]=await Promise.all([reloadWorld(),loadMaxRadius()]);customizeGlobalCopy();
    if(status&&$('#snArIntro')?.classList.contains('show'))setText(status,points.length?`✅ ${points.length} actieve AR Snazzle${points.length===1?'':'s'} op de kaart · zoeken tot maximaal ${radiusLabel()}.`:'ℹ️ Er staan nu geen actieve AR Snazzles op de kaart.');
  }catch{}
}
...
async function startGlobal(e){
  ...
  try{
    ...
    const [,pos,points,maxKm]=await Promise.all([cameraTask,geoQuick(),reloadWorld(),loadMaxRadius()]);
    if(!points.length)throw new Error('Er staan nu geen actieve AR Snazzles op de kaart.');
    const here=point(pos),sorted=points.slice().sort((a,b)=>dist(here,a)-dist(here,b)),nearest=sorted[0],nearestMeters=dist(here,nearest);
    if(nearestMeters>maxKm*1000)throw new Error(`Geen actieve Snazzle binnen ${radiusLabel(maxKm)}. Bekijk de kaart voor andere Snazzle-zones.`);
    target=nearest;setTargetVisual();setVisible(false);
    const remaining=Math.round(nearestMeters),accuracy=Math.round(Number(pos.coords.accuracy||0));
...
function repair(){installGeneralAdminOption();installMaxRadiusAdmin();lockArAdminToHeadAdmin();watchAdminRole();ensureGlobalMapLink();customizeGlobalCopy();installGlobalController();}
...
loadMaxRadius().then(customizeGlobalCopy).catch(()=>{});
window.SnazzleArGlobalV240={repair,refreshIntroStatus,stop:stopGlobal,loadMaxRadius};

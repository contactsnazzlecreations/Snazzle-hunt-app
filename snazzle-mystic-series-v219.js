// Snazzle MYSTIC Series 01 v235 — 12 kaarten, artwork en serie-mappen volgens vaste 48-kaartenstructuur.
...
const defs=[
  ...
  ['Star Oracle','core'],['Dream Keeper','core'],['Phantom Flash','rare'],['Magic Mist','rare'],
  ['Lunar Legend','silver'],['Secret Spirit','silver'],['Mystic Guardian','gold'],['Mystic Master','platinum']
];
let art=[];
...
function canonical(i,id=cardId(i),old={}){
  const [name,rarity]=defs[i];
  return {id,number:cardNo(i),name,series:'MYSTIC Series 01',description:'Magie, maanlicht & mysterie',rarity,
    unlockType:'event',huntId:old.huntId||'',threshold:0,imageData:art[i]||old.imageData||'',active:true,secretName:false,
...
function seedLocal(){
  ...
  const before=loadLocal(),byNo=new Map(before.map(c=>[String(c.number||'').toUpperCase(),c]));
  const mysticNos=new Set(defs.map((_,i)=>cardNo(i))),keep=before.filter(c=>!mysticNos.has(String(c.number||'').toUpperCase()));
  const made=defs.map((_,i)=>{const old=byNo.get(cardNo(i))||{};return {...old,...canonical(i,old.id||cardId(i),old),createdAt:old.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()}});
  localStorage.setItem(LOCAL_KEY,JSON.stringify([...keep,...made]));return true;
}
...
async function syncCentral(u){
  if(syncBusy||!art.length||!(await isSuperAdmin(u)))return;syncBusy=true;
  try{
    const snap=await getDocs(collection(db,'snazzleCards')),byNo=new Map(snap.docs.map(d=>[String(d.data().number||'').toUpperCase(),d]));
    const batch=writeBatch(db),now=new Date().toISOString();
    defs.forEach((_,i)=>{const found=byNo.get(cardNo(i)),id=found?.id||cardId(i),old=found?.data?.()||{};batch.set(doc(db,'snazzleCards',id),{...canonical(i,id,old),createdAt:old.createdAt||now,updatedAt:now},{merge:true});});
    await batch.commit();window.dispatchEvent(new CustomEvent('snazzle:mystic-ready',{detail:{version:VERSION,count:12}}));
  }catch(e){console.warn('MYSTIC centrale sync wacht op beheerrechten',e)}finally{syncBusy=false}
}
...
function ensureSeriesPills(){const filters=$('#sc2Filters');if(!filters)return;let wrap=$('#sc2SeriesFolders');if(!wrap){wrap=document.createElement('div');wrap.id='sc2SeriesFolders';wrap.className='sc-series-folders';filters.parentNode?.insertBefore(wrap,filters)}if(!wrap.dataset.ready){const items=[['ALL','Alle series'],['WILD','🌿 WILD'],['SPARK','✨ SPARK'],['MYSTIC','🔮 MYSTIC'],['BLAZE','🔥 BLAZE']];wrap.innerHTML=items.map(([k,l])=>`<button type="button" class="sc-series-pill" data-seriespick="${k}">${l}</button>`).join('');$$('[data-seriespick]',wrap).forEach(b=>b.onclick=()=>{selectedSeries=b.dataset.seriespick;ensureSeriesPills();applyCollectionFilter()});wrap.dataset.ready='1'}$$('[data-seriespick]',wrap).forEach(b=>b.classList.toggle('on',selectedSeries===b.dataset.seriespick));}
function applyCollectionFilter(){const g=$('#sc2Grid');if(!g)return;$$(':scope > .sc2-card',g).forEach(c=>{const s=seriesFromCard(c);c.dataset.seriesV219=s;c.style.display=(selectedSeries==='ALL'||selectedSeries===s)?'':'none';});}
...
function repairUI(){addStyles();ensureSeriesPills();applyCollectionFilter();groupAdmin();window.SnazzleCardProgressV234?.render?.();}
...
(async()=>{try{await loadAtlas();const first=localStorage.getItem(MARKER)!=='1';seedLocal();localStorage.setItem(MARKER,'1');if(first){setTimeout(()=>location.reload(),180);return}repairUI();if(auth)onAuthStateChanged(auth,u=>{if(u)syncCentral(u)})}catch(e){console.error('MYSTIC Series 01 v235 kon niet laden',e)}})();

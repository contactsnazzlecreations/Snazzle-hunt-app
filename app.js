// Snazzle Hunt v292 — Special Snazzles tonen automatisch de plaats waar ze gevonden zijn.
...
function preloadCriticalAssets(){
  const moduleHrefs=[
    ...
    './snazzle-clean-home-v31.js?v=292',
    './snazzle-home-card-backgrounds-v75.js?v=292'
  ...
  ['./snazzle-reference-layout.css?v=292','./snazzle-clean-home-v31.css?v=292'].forEach(href=>{
...
async function optionalImport(path,label){
  try{
    ...
    return await import(`${path}${path.includes('?')?'&':'?'}v=292`);
...
await import('./app-runtime-v245.js?v=292');
...
document.addEventListener('click',event=>{
  const shopTarget=event.target?.closest?.('#navShop,[data-quick-action="shop"]');
  if(!shopTarget)return;
  event.preventDefault();
  event.stopPropagation();
  event.stopImmediatePropagation?.();
  window.location.href=EXTERNAL_SHOP_URL;
},{capture:true});

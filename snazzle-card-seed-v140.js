// Snazzle Cards v140 — veilige vaste basiscollectie zonder canvas of DOM-hertekenen.
// Zet 12 WILD + 12 SPARK kaarten als gewone catalogusrecords klaar.
// De kaartafbeeldingen worden alleen als CSS-sprite getoond; dit raakt menu/navigatie niet.
import { assets } from './snazzle-card-assets-v133.js';

const KEY='snazzleCardCatalogV2';
const VERSION='140-safe-seed';
...
function decorate(){
  const grid=document.getElementById('sc2Grid');if(!grid)return;
  grid.querySelectorAll('.sc2-card').forEach(card=>{
    const num=card.querySelector('.sc2-num')?.textContent?.trim();
    const info=spriteInfo(num);if(!info)return;
    const media=card.querySelector('.sc2-media');if(!media)return;
    media.classList.add('sn-seed-sprite');
    media.style.backgroundImage=`url(${JSON.stringify(info.src)})`;
    media.style.backgroundSize=`${info.cols*100}% ${info.rows*100}%`;
    ...
    const img=media.querySelector('img');if(img)img.style.opacity='0';
  });
}
function installStyle(){
  if(document.getElementById('snCardSeedV140Style'))return;
  const s=document.createElement('style');s.id='snCardSeedV140Style';
  s.textContent=`.sc2-media.sn-seed-sprite{background-color:#173d2d!important}.sc2-card.locked .sc2-media.sn-seed-sprite:before{content:'';position:absolute;inset:0;background:rgba(4,13,10,.76);z-index:1}.sc2-media.sn-seed-sprite .sc2-lock,.sc2-media.sn-seed-sprite .sc2-rarity,.sc2-media.sn-seed-sprite .sc2-num{z-index:2}`;
  document.head.appendChild(s);
}
installStyle();

// Alleen decoreren wanneer het kaartenvak bestaat of verandert; nooit de pagina opnieuw tekenen.
const observer=new MutationObserver(()=>decorate());
function attach(){
  const grid=document.getElementById('sc2Grid');
  if(grid&&!grid.dataset.seed140Observed){grid.dataset.seed140Observed='1';observer.observe(grid,{childList:true,subtree:true});decorate();}
}
document.addEventListener('click',e=>{
  if(e.target.closest('[data-collection-tab],#openCollection,.collection-tabs'))setTimeout(()=>{attach();decorate();},60);
},{passive:true});
